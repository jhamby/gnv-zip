$! File: PCSI_PRODUCT_ZIP.COM
$!
$! This command file packages up the product ZIP into a sequential
$! format kit
$!
...
$ if kit_name .eqs. ""
$ then
$   write sys$output "@[.vms]make_pcsi_zip_kit_name.com has not been run."
$   goto all_exit
$ endif
...
$ src6 = "sys$disk:[''my_dir'],prj_root:[''my_dir'.src],"
$ src7 = "new_gnu:[usr.share.doc.''project_name'],"
$ src8 = "new_gnu:[usr.share.doc.man.man1],"
$ src9 = "sys$disk:[''my_dir'.man],sys$disk:[''my_dir'.man.man1]"
...
$!
$!
$ ! Look for real GNV zip program
$ gnv_zip = f$search("GNV$GNU:[usr.bin]gnv$zip.exe")
$ if gnv_zip .eqs. ""
$ then
$   ! Look for the zip we just created
$   gnv_zip = f$search("GNV$GNU:[bin]gnv$zip.exe")
$ endif
$ if gnv_zip .nes. ""
$ then
$   zip := $'gnv_zip'
